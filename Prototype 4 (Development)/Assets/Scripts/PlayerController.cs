...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (effectManager.playerRevive.isEffectActivated)
            {
                RespawnPlayer();
                effectManager.playerRevive.DisableEffectNow();
                //effectManager.playerRevive.isEffectActivated = false; // 한 번 리스폰되면 버프2 상태 해제
            }
            else
            {
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("DeBuff"))
        {
            ...
            effectManager.enemyKnockbackHard.ActiveEffect();
        ...
        {
            Destroy(other.gameObject);
            effectManager.enemyHeavy.ActiveEffect();
        ...
        {
            Destroy(other.gameObject);
            effectManager.playerPushing.ActiveEffect();
            //ActivateBuff();
        }
